      ...
      {showGround && (
        <group position={[0, 0.25, 0]}>
          ...
          <mesh position={[0.5, 2.4, 4.1]}>
            ...
            <meshPhysicalMaterial color="#BAE6FD" transmission={0.7} opacity={0.8} transparent roughness={0.1} />
          </mesh>

          {/* Warm Interior Lighting */}
          <pointLight position={[-1, 1.5, 0]} color="#FFEDD5" intensity={2.5} distance={8} />
        </group>
      )}
      ...
      {showFirst && (
        <group position={[0, 2.8, 0]}>
          ...
          <mesh position={[2.4, 0.7, 4.8]}>
            ...
            <meshPhysicalMaterial color="#BAE6FD" transparent opacity={0.6} roughness={0.1} />
          </mesh>
        </group>
      )}
  ...
  return (
    <div className="w-full h-full relative select-none bg-white">
      ...
      >
        ...
        <directionalLight position={[14, 20, 14]} intensity={2.0} color="#FFF7ED" castShadow />
        <directionalLight position={[-12, 12, -12]} intensity={0.5} color="#E0F2FE" />
      ...
      <div className="absolute top-4 left-4 pointer-events-none bg-white/95 backdrop-blur-md px-3.5 py-2 border border-gray-200 text-[11px] font-mono text-[#111827] uppercase flex items-center space-x-2 shadow-sm rounded-xs">
      ...
      <button
        ...
        className="absolute top-4 right-4 z-10 bg-white/95 hover:bg-[#FFF7ED] active:scale-95 transition-all backdrop-blur-md px-3 py-1.5 border border-gray-200 text-[11px] font-mono tracking-wider text-[#111827] uppercase flex items-center space-x-2 shadow-xs cursor-pointer rounded-xs"
        title="Toggle 360° Auto-Rotation"
        aria-label="Toggle 360° Auto-Rotation"
      >
        <span className={`w-2 h-2 rounded-full transition-colors ${autoRotate ? 'bg-[#EA580C] animate-pulse' : 'bg-gray-400'}`} />
        <span className="font-semibold text-[#111827]">{autoRotate ? '360° ROTATING' : 'ROTATION PAUSED'}</span>
      </button>
    </div>
  );
}
